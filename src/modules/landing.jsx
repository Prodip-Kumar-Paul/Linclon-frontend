...
const Landing = () => {
  ...
  const name="NAME";
  const language=[["JAVASCRIPT"],["C++"],["BLOCKCHAIN"],["REACT Js"],["ANGULAR Js"],["GO"],["CSS"],["MERN"],["MEAN"],["MONGO DB"],["PYTHON"],["JAVA"],["RUBY"],["SQL"],["C#"],["Any Other?"]];
return <>
   <div className='my-[2rem] mx-[5%] py-[3rem] bg-white rounded-lg dark:bg-gradient-to-r dark:from-[rgb(50,50,50)] dark:to-[rgb(40,40,40)] dark:text-white overflow-hidden'>
      <section className='flex flex-col md:text-2xl xs:text-thin mb-[5rem]'>
        <article className='items-center flex md:flex-row xs:flex-col-reverse bg-white rounded-lg shadow-2xl'>
            <section className='flex flex-col ml-[5%] mt-[2rem]'>
              ...
              <br/>
              <p className='text-violet-900 border-l pl-5 '>Build your First Project with LINCLON as a Contributor And Execute / improve your skills. </p>
                <Link to={"/projects"} className='self-start ml-5 bg-cyan-500 rounded-lg px-2 py-1 my-2 text-white text-[1rem]'>
                Get Started
                </Link>
            </section>
        ...
        <article className='flex'>
          <img alt="" src={connecting} className='min-h-[50%] w-[50%] rounded-full'/>
          ...
          Lorem ipsum, dolor sit amet consectetur Lorem ipsum dolor sit amet Lorem ipsum dolor sit amet..</p>
        </article>
        <article className='flex'>
          <p className='grow self-center  w-[30%] text-cyan-500 mx-[10%]'>
            <span className=" dark:text-neutral-100 text-black font-bold">
           ...
           Lorem ipsum Voluptates voluptatibus voluptas voluptatum vitae quo. Magni laudantium eos corrupti dolor facilis?</p>
          <img alt="" src={team} className='min-h-[50%] w-[50%] rounded-full'/>
        </article>
        <article className='flex'>
          <img alt="" src={creator} className='min-h-[50%] w-[50%] rounded-full'/>
          <p className='grow self-center  w-[30%] text-cyan-500 mx-[10%]'>
            <span className=' dark:text-neutral-100 text-black font-bold'>
      ...
      <div className='flex flex-row gap-5 justify-center max-h-[25rem] mx-[5%] border-2 rounded-lg flex-wrap overflow-auto'>
        { language.map((card,index)=>{
          return (
          <section key={card} className='text-center text-white max-w-[12rem] min-h-[12rem] mx-4 px-2 py-2 my-4 bg-cyan-500 rounded-[0.25rem]'>
           {card}
           <p className='text-black'>Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.</p>
          </section>
        ); })}
      </div>
