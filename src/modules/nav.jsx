...
const Nav = () => {
   ...
   let themeChange = () => {
      if (themeIcon) {
         ...
         document.body.style.backgroundImage =
            "linear-gradient(rgb(192,192,192),rgb(192,192,192))";
      } else {
         setThemeIcon(true);
   ...
   const isSignInModal = () => {
      if (signInModal) setSignInModal(false);
   ...
   return (
      <>
         ...
         <div
            id="modalId"
            style={signInModal ? {} : { display: "none" }}
            className="z-[10] mt-[3.25rem] h-[100%] w-[100%] fixed bg-[rgba(0,0,0,0.6)]"
         >
            <section className="relative font-bold rounded-[.5rem] max-w-[25rem] mx-auto mt-[5rem] bg-white dark:bg-black dark:text-white text-lg">
               <span
                  onClick={isSignInModal}
                  className=" dark:text-white absolute right-2 cursor-pointer"
               >
                  &times;
               </span>
               <br />
